...
function Mainpage() {
  ...
  const getResult = () => {
    ...
    setLoader(true);
    axios
      .post("http://192.168.0.108:4000/getsummary", userRequest)
      .then((res) => {
        const data = res?.data?.data;
        const new_ip = "192.168.0.110";

        data["matchingPageUrl"] = data["matchingPageUrl"].replace(
          "localhost",
          new_ip
        );
          ...
          <div className="row">
            <div className="col-md-6">
              <div className="h-100 p-5 text-bg-dark rounded-3">
                <h2>{quest}</h2>
                <p>
                  The human eye is a special part of our body that helps us see
                  the world around us. It is like a camera that takes pictures
                  and sends them to our brain. The eye has a round shape and is
                  located in our face. It has a colored part called the iris,
                  which can be blue, green, brown, or other colors. The iris
                  controls how much light enters the eye. Inside the eye, there
                  is a lens that focuses the light onto a special part called
                  the retina. The retina has cells that can detect light and
                  send signals to our brain. Our brain then interprets these
                  signals and helps us understand what we are seeing
                </p>
